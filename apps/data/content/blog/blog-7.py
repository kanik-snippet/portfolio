"""
Blog Post #7: Eid al-Fitr: Celebrating the Prophet's Way
Generated from centralized blog data
"""

from datetime import datetime
from django.conf import settings

# Blog data for: Eid al-Fitr: Celebrating the Prophet's Way
blog_data = {
    "id": 7,
    "title": """Eid al-Fitr: Celebrating the Prophet's Way""",
    "description": """Dive into how Prophet Muhammad (PBUH) rocked Eid al-Fitr with joy, faith, and community love.""",
    "images": {
        "eid_al_fitr_prophetic_way.webp": f"{settings.BLOG_BASE_IMG_URL}/eid_al_fitr_prophetic_way.webp"
    },
    "created_at": datetime.strptime("2025-03-31T00:00:00+07:00", "%Y-%m-%dT%H:%M:%S%z"),
    "updated_at": datetime.strptime("2025-07-22T14:29:00+07:00", "%Y-%m-%dT%H:%M:%S%z"),
    "author": "Kanik Gupta",
    "username": "roneha",
    "author_image": settings.AUTHOR_IMG,
    "content": [
        {
            "type": "p",
            "class": "mb-4 text-sm md:text-base lg:text-lg",
            "text": "Eid al-Fitr represents one of Islam's most joyous celebrations, marking the conclusion of Ramadan's month-long fast. Understanding how Prophet Muhammad (peace be upon him) celebrated this occasion provides valuable guidance for Muslims seeking to honor this special day according to authentic traditions."
        },
        {
            "type": "h2",
            "class": "text-xl lg:text-2xl text-medium mt-4 md:mt-5 mb-2 md:mb-3",
            "text": "The Prophetic Spirit of Eid Celebration"
        },
        {
            "type": "p",
            "class": "mb-4 text-sm md:text-base lg:text-lg",
            "text": "Eid al-Fitr back in Prophet Muhammad's (PBUH) day was all about gratitude and good times. It kicked off with a big group prayer outdoors, followed by a sermon that lit up everyone's faith and unity. This communal celebration emphasized the importance of collective joy and spiritual renewal."
        },
        {
            "type": "h2",
            "class": "text-xl lg:text-2xl text-medium mt-4 md:mt-5 mb-2 md:mb-3",
            "text": "Spiritual Practices and Traditions"
        },
        {
            "type": "p",
            "class": "mb-4 text-sm md:text-base lg:text-lg",
            "text": "The Prophet pushed takbir—glorifying Allah from Eid's eve till prayer time—to keep the thankful vibes flowing. Zakat al-Fitr was a must, making sure everyone, especially the less fortunate, could join the party. These practices ensured that spiritual devotion remained central to the celebration."
        },
        {
            "type": "h2",
            "class": "text-xl lg:text-2xl text-medium mt-4 md:mt-5 mb-2 md:mb-3",
            "text": "Personal Preparations and Community Building"
        },
        {
            "type": "p",
            "class": "mb-4 text-sm md:text-base lg:text-lg",
            "text": "Before heading out, he'd munch on dates to mark the end of fasting—a small move with big meaning. Eid was also about patching things up, forgiving, and spreading kindness to make the community tight. The Prophet emphasized that personal joy should extend to strengthening bonds with family, friends, and the broader community."
        },
        {
            "type": "h2",
            "class": "text-xl lg:text-2xl text-medium mt-4 md:mt-5 mb-2 md:mb-3",
            "text": "Implementing Prophetic Eid Practices Today"
        },
        {
            "type": "p",
            "class": "mb-4 text-sm md:text-base lg:text-lg",
            "text": "Contemporary Muslims can honor the Prophetic tradition by maintaining the balance between celebration and spiritual significance. This includes participating in community prayers, ensuring charitable giving reaches those in need before the Eid prayer, wearing one's best clothes, and fostering an atmosphere of reconciliation and happiness that extends beyond one's immediate circle."
        }
    ],
    "is_featured": False,
    "tags": ['Eid al-Fitr', 'Prophet Muhammad', 'Islam', 'Community'],
    "category": "",
    "read_time": 5,
    "views": 0,
    "slug": ""
}
